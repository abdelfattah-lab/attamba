...
from apps.attamba.generate import (
    PackedCausalAttambaGenerator,
    PackedCausalAttambaGeneratorArgs,
    load_consolidated_model_and_tokenizer,
)

from apps.attamba.attamba import LMAttamba, LMAttambaArgs
...
class EvalArgs:
    ...
    ckpt_dir: str = ""
    generator: PackedCausalAttambaGeneratorArgs = field(
        default_factory=PackedCausalAttambaGeneratorArgs
    )
...
def launch_eval(cfg: EvalArgs):
    ...
    model, tokenizer = load_consolidated_model_and_tokenizer(
        consolidate_path, LMAttamba, LMAttambaArgs
    )
    logger.info("Model loaded")
    model.eval()
    generator = PackedCausalAttambaGenerator(cfg.generator, model, tokenizer)
...
def main():
    ...
    Then you can pass model.dim=32 to change values in LMAttambaArgs

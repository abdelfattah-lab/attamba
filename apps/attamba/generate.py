...
class PackedCausalAttambaGenerator:
    def __init__(
        self,
        cfg: PackedCausalAttambaGeneratorArgs,
        model: nn.Module,
        tokenizer: Tokenizer,
    ):
        """
...
def main():
    ...
    gen_cfg = dataclass_from_dict(
        PackedCausalAttambaGeneratorArgs, cfg, strict=False
    )
    print(cfg)

    model, tokenizer = load_consolidated_model_and_tokenizer(cfg.ckpt)

    generator = PackedCausalAttambaGenerator(gen_cfg, model, tokenizer)

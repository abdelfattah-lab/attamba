...
def download_dataset(repo_id, local_dir, allow_patterns):
    allow_patterns = "*.jsonl.zst"  # Adjust to filter further if needed
    if "dclm" in repo_id:
        allow_patterns = [
    ...
    print(f"Downloading dataset from {repo_id} with filter {allow_patterns}...")
    snapshot_download(
        repo_id,
        repo_type="dataset",
        local_dir=local_dir,
...
def main(dataset, memory, data_dir, seed=42):
    ...
    repo_id = {
        ...
        # "dclm_baseline_1.0": "mlfoundations/dclm-pool-400m-1x",
    }[dataset]
